...
function generarPDF(boleta) {
    ...
    boleta.items.forEach(item => {
        doc.text(`Producto: ${item.titulo}`, 20, y);
        doc.text(`Precio: ${item.precio}`, 120, y);
        doc.text(`Cantidad: ${item.cantidad}`, 160, y);
        y += 10;
    });

    doc.setFontSize(16);
    doc.text(`Total: ${boleta.total}`, 20, y + 20);

    doc.save('boleta.pdf');
}
document.addEventListener("DOMContentLoaded", function() {
    // Obtén todos los elementos de redes sociales
    const redesSociales = document.querySelectorAll('.red-social a');

    // Itera sobre cada enlace y agrega el evento clic
    redesSociales.forEach(function(red) {
        red.addEventListener('click', function(event) {
            // Evita el comportamiento predeterminado del enlace
            event.preventDefault();
            
            // Obtén el enlace del atributo href
            const url = red.getAttribute('href');
